...
func TestArchRepository_Get(t *testing.T) {
	...
	defer ctrl.Finish()
	logger := mock_log.NewMockLogger(ctrl)
	query := "SELECT arches.id, arches.name, arches.release, arches.tested, arches.updated_at, arches.created_at FROM public.arches WHERE id = \\$1 LIMIT 1"
	arch := mock_models.NewArch(t)
	ctx := context.Background()
	type fields struct {
		database *sqlx.DB
...
func TestArchRepository_List(t *testing.T) {
	...
	query := "SELECT arches.id, arches.name, arches.release, arches.tested, arches.updated_at, arches.created_at FROM public.arches"
	type fields struct {
		database *sqlx.DB
		logger   log.Logger
	}
